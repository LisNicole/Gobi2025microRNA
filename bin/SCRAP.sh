...
#Unzip FASTQ file from Cutadapt
      ...
      # Unzip the FASTQ file from Cutadapt (if required by your deduplication step)
    gunzip -k "${directory}${sample}/${sample}.cutadapt.fastq.gz"

    # Run FastQC on the final Cutadapt file (before deduplication)
    mkdir -p "${directory}FastQC_PostCutadapt"
    fastqc \
      -o "${directory}FastQC_PostCutadapt" \
      "${directory}${sample}/${sample}.cutadapt.fastq.gz"
...
if [ "$pre_filtered" == "yes" ]; then
    ...
    echo "${sample} pre-miRNA filtered: $(( $(wc -l < "${directory}${sample}/${sample}.premiRNA.removed.fasta") / 2 ))" >> "${directory}${sample}/${sample}.summary.txt"
    ##########
    ...
    echo "${sample} tRNA filtered: $(( $(wc -l < "${directory}${sample}/${sample}.tRNA.removed.fasta") / 2 ))" >> "${directory}${sample}/${sample}.summary.txt"
    ##########
...
# Merge miRNA and hairpin alignments for isomiR analysis
join \
...
# Join the miRNA alignment (field 13) with the hairpin alignments (field 2)
# (Note: Using explicit sort key ranges to ensure proper ordering)
join -1 13 -2 2 \
    <(sort -k13,13 "${directory}${sample}/${sample}.blast.miRNA_iso.plus.merged") \
    <(sort -k2,2 "${directory}${sample}/${sample}.hairpin.miRNA.blast") \
    | awk '{if($3==$24) print $0}' | sort -k2,2 \
    > temp1.plus

join -1 13 -2 2 \
    <(sort -k13,13 "${directory}${sample}/${sample}.blast.miRNA_iso.minus.merged") \
    <(sort -k2,2 "${directory}${sample}/${sample}.hairpin.miRNA.blast") \
    | awk '{if($3==$24) print $0}' | sort -k2,2 \
    > temp1.minus
...
awk 'BEGIN{RS=">"; OFS="\t"} NR>1 {print $1, $2}' "${directory}${sample}/${sample}.cutadapt.deduped.barcoded.fasta" \
...
# (If desired, remove the miRNA alignments and append the new miRNA/isomiR alignments here)
...
echo "${sample} filtered BLAST alignments: $(( $(wc -l < "${directory}${sample}/${sample}.blast.merged") ))" >> "${directory}${sample}/${sample}.summary.txt"
##########
...
echo "${sample} potential chimeras: $(( $(wc -l < "${directory}${sample}/${sample}.target.fasta") / 2 ))" >> "${directory}${sample}/${sample}.summary.txt"
##########
...
echo "${sample} unique alignments: $(( $(awk '$5 == 60' "${directory}${sample}/${sample}.aligned.sam" | wc -l) ))" >> "${directory}${sample}/${sample}.summary.txt"
##########
