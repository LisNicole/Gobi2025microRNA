...
then
  ...
  awk '!/miRNA-/' ${directory}PeakCalling/${sample}.aligned.unique.sam \
    > ${directory}${sample}/${sample}.sam.nonmiRNA.txt


  awk '/miRNA-/' ${directory}PeakCalling/${sample}.aligned.unique.sam | sed 's/\.miRNA-/\.miRNA-\t/' | sort -k2,2 > temp_mirna_sorted.txt

  grep "mmu-" ${reference_directory}annotation/miR_Family.txt  | sort -k1,1> temp_mir_family_sorted.txt

  join -a 1 -1 2 -2 1 temp_mirna_sorted.txt temp_mir_family_sorted.txt > temp_joined.txt

  awk '{$1=""}1' temp_joined.txt | \
  awk '{$1=$1}1' | \
  awk '{$1 = $1$NF; print}' | \
  awk 'NF{NF-=1};1' | \
  sed 's/ /\t/g' > ${directory}${sample}/${sample}.aligned.unique.tmp.sam
...
#If not calling peaks by family, move and rename sam file to peak calling folder
	...
	cp ${directory}PeakCalling/${sample}.aligned.unique.sam ${directory}PeakCalling/${sample}.aligned.unique.tmp.sam
...
#Convert sam file to bam file
	...
	samtools sort -o ${directory}PeakCalling/${sample}.aligned.unique.sorted.bam \
  ${directory}PeakCalling/${sample}.aligned.unique.bam
  mv ${directory}PeakCalling/${sample}.aligned.unique.sorted.bam \
  ${directory}PeakCalling/${sample}.aligned.unique.bam

  samtools index ${directory}PeakCalling/${sample}.aligned.unique.bam
...
#Replace temporary bed file with bed file that reads were removed from
  ...
  wc -l ${directory}PeakCalling/combined.rearranged.sorted.tmp.bed
	echo "Cycle complete" >> ${directory}PeakCalling/cycles.txt
  echo "value of x "
  wc -l ${directory}PeakCalling/combined.rearranged.sorted.merged.coverage | awk '{print $1}'
done
...
then
  ...
  touch "${directory}peakcalling.summary_c${minimum_reads}_l${minimum_libraries}.txt"
  peak_summary_file="${directory}peakcalling.summary_c${minimum_reads}_l${minimum_libraries}.txt"
  echo "Finish: $(date)" >> "$peak_summary_file"
  echo "Samples:" >> "$peak_summary_file"
  echo "${samples}" >> "$peak_summary_file"
  echo "miRBase species abbreviation: ${miRBase_species_abbreviation}" >> "$peak_summary_file"
  echo "Genome species abbreviation: ${genome_species_abbreviation}" >> "$peak_summary_file"
  echo "Peaks called by family? ${family}" >> "$peak_summary_file"
  echo "Minimum reads: ${minimum_reads}" >> "$peak_summary_file"
  echo "Minimum libraries: ${minimum_libraries}" >> "$peak_summary_file"
  echo "Cycles complete: $(wc -l ${directory}PeakCalling/cycles.txt | awk '{print $1}')" >> "$peak_summary_file"
  echo "Total peaks: $(wc -l ${directory}PeakCalling/peaks.bed | awk '{print $1}')" >> "$peak_summary_file"

  if [ "$family" == "yes" ]; then
      mv "$peak_summary_file" "${directory}peakcalling.family.summary_c${minimum_reads}_l${minimum_libraries}.txt"
  else
