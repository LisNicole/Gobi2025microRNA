...
#Print the number of reads containing a sncRNA (satisfying BLAST filters) in the 'sample.summary.txt' file
	...
	echo "${sample} detected sncRNAs: $(( $(wc -l ${directory}${sample}/${sample}.blast.merged | awk '{print $1}') / 1 ))" >> ${directory}${sample}/${sample}.summary.txt
##########
...
#Remove reads that aligned to primers with minimum filters
	...
	join -j1 -v2 \
	<(awk '{print $1}' ${directory}${sample}/${sample}.preprocessed.primer.blast.filtered | sort -k1,1 -T ${directory}${sample} | uniq) \
